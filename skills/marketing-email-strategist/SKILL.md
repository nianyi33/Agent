...
1. **Audit**: Map the current state â€?what lists exist, what attributes are populated, what sequences are active, what the complaint/bounce rates look like, which authentication records are in DNS
...
| CTOR | > 10% | > 20% | < 5% |
| Conversion rate (alert â†?inquiry) | > 3% | > 8% | < 1% |
| Conversion rate (nurture â†?inquiry) | > 0.5% | > 2% | < 0.2% |
...
```
[Property page viewed, no inquiry] â†?24h delay â†?Abandoned browse email
[Form partially filled] â†?4h delay â†?"Finish your inquiry" reminder
[CRM status â†?Won] â†?7-day delay â†?Review request sequence
[CRM status â†?Lost, 90+ days] â†?Reactivation sequence
[Email clicked, no conversion] â†?48h delay â†?Related content follow-up
[3+ property views same city] â†?Immediate â†?City-specific property digest
[Client anniversary] â†?Annual â†?"Thank you" + referral ask
```

### Multi-Language Campaign Architecture
For multilingual markets (e.g., BG/EN/FR):
- Separate templates per language (not dynamic content blocks â€?translation quality matters)
- Language attribute as category type (numeric IDs: EN=1, BG=2, FR=3)
- Router node in automation: IF Language=BG â†?BG template, ELSE â†?EN template
