# TreeSloth AI Agent 项目功能说明

## 1. 项目定位

TreeSloth AI Agent 是一个持续运行的桌面 AI Agent 项目。它不是传统意义上的网页聊天机器人，也不是一次问答结束就停止工作的命令行工具，而是一个运行在本机桌面上的长期智能体框架。

它的核心目标可以概括为一句话：

> 让 AI 从“会回答问题的聊天框”，进化成“常驻本地、能记住、能行动、能展示、能自我维护的桌面智能体”。

TreeSloth AI Agent 由 Electron 桌面应用、本地 HTTP 服务、LLM 调用层、动态记忆系统、工具执行系统、语音系统、社交连接器、Brain UI、Scene UI 协议和本地持久化数据库组成。它既可以处理用户即时消息，也可以在空闲时通过心跳机制继续整理记忆、检查任务、刷新上下文、执行提醒、采集热点和维护自身状态。

发布会中建议把 TreeSloth AI Agent 讲成“三层能力”的结合：
...
传统 AI 产品通常以“输入一句话，输出一段回答”为中心。TreeSloth AI Agent 的设计不是围绕单次问答，而是围绕一个持续运行的 Agent：
...
TreeSloth AI Agent 的重要设计理念是 ACI（Anticipatory Context Injection，预判注入）。

传统 Agent 常常等模型发现自己需要资料，再调用工具，再等待结果。TreeSloth AI Agent 在模型开口之前，就会主动准备可能需要的上下文：
...
TreeSloth AI Agent 不是让 Agent 直接操作 DOM、写 CSS 或控制像素，而是让 Agent 声明“当前屏幕上应该有什么语义状态”。
...
这个设计让 TreeSloth AI Agent 具备未来扩展多套界面的基础：
...
1. **项目协作场景**  
   用户让 TreeSloth AI Agent 阅读项目文件、分析架构、生成发布会文档、运行测试、整理变更说明。Agent 会读取仓库、召回记忆、调用文件和 Shell 工具，并在必要时打开可视化进度窗口。

2. **长期任务场景**  
   用户设置一个多步骤目标，TreeSloth AI Agent 持久化任务状态。即使应用重启，也可以恢复当前任务和步骤，并通过 Tick 机制继续推进。

3. **桌面助手场景**  
   用户询问本机软件、桌面文件、系统信息、网络代理、已安装工具等问题，TreeSloth AI Agent 启动时已经扫描本机环境，可直接结合本地证据回答。

4. **语音陪伴场景**  
   用户通过语音输入唤醒和对话，TreeSloth AI Agent 可以用 TTS 自动朗读回复，支持音色、语速、输出设备和打断逻辑配置。
...
从架构上看，TreeSloth AI Agent 的每一轮工作大致分为七步：
...
Brain UI 是 TreeSloth AI Agent 的主要用户界面，入口为本地服务 `/brain-ui`。它不是单纯的聊天窗口，而是一个认知可视化界面。
...
这个模块是 TreeSloth AI Agent 与普通聊天应用最明显的差异之一：它让用户看到 AI 不是“只在你敲回车时存在”。
...
TreeSloth AI Agent 的 UI 里有多种专项面板：

- **热点面板**：展示抖音、小红书、微信热点、微博等趋势内容。
- **世界杯面板**：展示世界杯赛程、比分、积分榜和赛况。
- **文档面板**：展示模型配置、语音配置、微信/社交配置、TreeSloth AI Agent 架构和 UI 设计说明。
...
TreeSloth AI Agent 的主循环在 `src/index.js` 中实现。启动后，它会：
...
- 首次启动或特定版本自检时，检查环境、资源、工具和 UI。
- 初始若干个 Tick 内处于觉醒阶段。
- 觉醒阶段可以读取已有记忆、查找未完成线索，但不会主动给用户发起打扰式消息。
- 觉醒进展可通过 `awakening` scene surface 展示。
...
TreeSloth AI Agent 使用本地 SQLite 数据库存储长期状态，主要包括：
...
TreeSloth AI Agent 有 threads / commitments 线索模型：
...
ACI 是 TreeSloth AI Agent 的关键智能机制。
...
TreeSloth AI Agent 不会每轮把所有工具 schema 都注入给模型，而是根据：
...
TreeSloth AI Agent 的工具系统由 schema、执行器、沙箱、安全策略、行动日志和动态工具市场组成。
...
- `grant_agent_delegation`：记录用户是否允许 TreeSloth AI Agent 指挥其他本地 Agent。
...
这让 TreeSloth AI Agent 的能力不是固定死的，而是可以在运行中扩展。
...
TreeSloth AI Agent 支持云端 ASR 会话，前端通过 `/voice/cloud` WebSocket 接入。
...
打包后如果检测到 `portable.flag` 或设置 `TREESLOTH_AI_AGENT_PORTABLE_DIR`，可以将用户数据目录切到便携目录，方便 U 盘或独立目录运行。

### 19.4 日志落盘

桌面主进程会把 console 日志写入用户目录下的日志文件：

- `logs/treesloth-ai-agent.log`
- 超过 5MB 时轮转为 `treesloth-ai-agent.old.log`
...
可以设置 `TREESLOTH_AI_AGENT_API_TOKEN`。远程请求可通过 Bearer token 或 query token 访问受保护接口。
