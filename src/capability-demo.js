...
async function streamWriteDemoFile(id) {
  ...
  const article = [
    'TreeSloth像一个住在电脑里的行动伙伴。它不仅能回答问题，也能把能力投到屏幕上：查询天气时变成卡片，写文件时把文字一段段落到磁盘，运行命令时打开真实窗口，让过程被看见。它还能展开热点面板，替你整理当天值得关注的信息。无论是记录灵感、检查环境、生成内容，还是把复杂任务拆成一步步行动，它都应该让你看得见、听得懂、能掌控。好的助手不只是会说，更要能把事情做出来，并让你清楚地看到每一步。',
    '',
    `生成时间：${nowTimestamp()}`,
  ].join('\n')
...
function writeCmdDemoScript(windowTitle = 'TreeSloth AI Agent Capability Demo') {
...
function sanitizeCmdTitle(title = '') {
  return String(title || 'TreeSloth AI Agent Capability Demo')
    .replace(/[&|<>^"]/g, '')
    .slice(0, 96)
    .trim() || 'TreeSloth AI Agent Capability Demo'
}
...
function openRealCmdWindow() {
  ...
  closeCmdWindow()
  const windowTitle = sanitizeCmdTitle(`TreeSloth AI Agent Capability Demo ${Date.now()}`)
