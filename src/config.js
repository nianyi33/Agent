...
export const XINYUN_PROVIDER = 'xinyun'
export const XINYUN_BASE_URL = 'https://xinyuntoken.com/v1'
export const DEFAULT_XINYUN_MODEL = 'deepseek-v4-pro'

export const XINYUN_MODELS = [
  { id: 'deepseek-v4-pro',                     label: 'DeepSeek V4 Pro（推荐）',        deprecated: false },
  { id: 'deepseek-v4-flash',                    label: 'DeepSeek V4 Flash',               deprecated: false },
  { id: 'deepseek-v3.2',                        label: 'DeepSeek V3.2',                   deprecated: false },
  { id: 'glm-5.2',                              label: 'GLM-5.2',                         deprecated: false },
  { id: 'glm-5.1',                              label: 'GLM-5.1',                         deprecated: false },
  { id: 'glm-5',                                label: 'GLM-5',                           deprecated: false },
  { id: 'glm-4.7',                              label: 'GLM-4.7',                         deprecated: false },
  { id: 'glm-4.7-flash',                        label: 'GLM-4.7 Flash',                   deprecated: false },
  { id: 'glm-4.6',                              label: 'GLM-4.6',                         deprecated: false },
  { id: 'kimi-k2.6',                            label: 'Kimi K2.6',                       deprecated: false },
  { id: 'qwen3-coder-480b-a35b-instruct',       label: 'Qwen3 Coder 480B',                deprecated: false },
  { id: 'qwen3-235b-a22b',                      label: 'Qwen3 235B',                      deprecated: false },
  { id: 'minimax-m3',                            label: 'MiniMax M3（最新）',               deprecated: false },
  { id: 'minimax-m2.7',                          label: 'MiniMax M2.7',                     deprecated: false },
  { id: 'minimax-m2.5',                          label: 'MiniMax M2.5',                     deprecated: false },
  { id: 'mimo-v2.5-pro',                        label: 'MiMo V2.5 Pro',                   deprecated: false },
  { id: 'mimo-v2.5',                            label: 'MiMo V2.5',                       deprecated: false },
]
...
const PROVIDER_CONFIG = {
  [XINYUN_PROVIDER]: {
    ...
    envVar: 'XINYUN_API_KEY',
    models: XINYUN_MODELS,
    defaultModel: DEFAULT_XINYUN_MODEL,
  },
}
...
function normalizeModel(model) {
  ...
  return DEFAULT_XINYUN_MODEL
}

function isThinkingEnabledForModel(model) {
  return normalizeModel(model) !== 'deepseek-v3.2'
}
...
function loadFromEnv() {
  const key = process.env['XINYUN_API_KEY']
  if (key) {
    return {
      provider: XINYUN_PROVIDER,
      apiKey: key,
      model: normalizeModel(process.env.XINYUN_MODEL),
    }
  }
  return null
}
...
export async function prepareActivation({ provider = XINYUN_PROVIDER, apiKey, model }) {
  const normalizedKey = String(apiKey || '').trim()
  const normalizedModel = normalizeModel(model)

  if (normalizedKey.length < 8) {
    throw new Error('API key is invalid — must be at least 8 characters')
  }
  ...
  try {
    ...
    if (status === 401 || /unauthoriz|invalid.*api.*key|authentication/i.test(err.message)) {
      throw new Error('API key validation failed — please check that the key is correct')
    }
...
export function commitPreparedActivation(prepared) {
  ...
  const normalizedModel = normalizeModel(prepared.model)
  if (normalizedKey.length < 8) throw new Error('API key is invalid')

  applyConfig(XINYUN_PROVIDER, normalizedKey, normalizedModel)
  persistLlmProviderConfig({
    provider: XINYUN_PROVIDER,
    apiKey: normalizedKey,
    model: normalizedModel,
  ...
  writeActiveLlmProvider(XINYUN_PROVIDER)
...
export async function activate({ apiKey, model }) {
  const prepared = await prepareActivation({ apiKey, model })
...
export function getActivationStatus() {
  return {
    ...
    model: config.model,
    baseURL: config.provider === 'custom' ? config.baseURL : XINYUN_BASE_URL,
    models: XINYUN_MODELS,
    defaultModel: DEFAULT_XINYUN_MODEL,
  }
}
...
export function getProviderSummaries() {
  const stored = resolveStoredLlmForProvider(XINYUN_PROVIDER)
  return {
    xinyun: {
      label: 'Xinyun',
      models: XINYUN_MODELS,
      ...
      model: stored?.model ? normalizeModel(stored.model) : DEFAULT_XINYUN_MODEL,
    },
    custom: (() => {
...
export function switchProviderConfig({ model } = {}) {
  const stored = resolveStoredLlmForProvider(XINYUN_PROVIDER)
  if (!stored) throw new Error('No saved configuration. Enter the API key once to save it.')
  const nextModel = normalizeModel(model || stored.model)
  applyConfig(XINYUN_PROVIDER, stored.apiKey, nextModel)
  persistLlmProviderConfig({
    provider: XINYUN_PROVIDER,
    apiKey: stored.apiKey,
    model: nextModel,
    activatedAt: readLlmProviderConfig(XINYUN_PROVIDER)?.activatedAt,
  })
  writeActiveLlmProvider(XINYUN_PROVIDER)
  return { provider: XINYUN_PROVIDER, model: nextModel, models: XINYUN_MODELS }
}

export async function saveLLMSettings({ apiKey, model } = {}) {
  const trimmedKey = String(apiKey || '').trim()
  if (trimmedKey) {
    const prepared = await prepareActivation({ apiKey: trimmedKey, model })
    return commitPreparedActivation(prepared)
  }
  return switchProviderConfig({ model })
}
...
export function getVoiceConfig() {
  ...
  const result = { voiceProvider: stored.voiceProvider || 'whisper-cpp' }
...
export function getTTSConfig() {
  ...
  const llmKey = config.apiKey || ''
  return {
    ttsProvider:     stored.ttsProvider  || 'xinyun',
    ttsVoiceId:      stored.ttsVoiceId   || 'nova',
    aliyunKey:       { configured: !!(stored.aliyunKey) },
    xinyunKey:       { configured: !!llmKey },
...
export function getTTSCredentials() {
  ...
  try {
    ...
    stored = cfg?.tts || {}
    voiceStored = cfg?.voice || {}
  } catch {}
  // 自动降级策略：Piper 本地不可用时，优先阿里云（ASR Key 共用），其次芯云
  let provider = stored.ttsProvider || 'xinyun';
  const xinyunKey = config.apiKey || '';
  const aliyunKey = stored.aliyunKey || voiceStored.aliyunApiKey || '';

  if (provider === 'piper-local') {
    const piperExe = stored.piperExe
      || (process.env.APPDATA ? path.join(process.env.APPDATA, 'AI_Agent', 'piper', 'piper.exe') : '');
    if (!piperExe || !fs.existsSync(piperExe)) {
      if (aliyunKey) {
        console.log('[TTS] piper.exe 不可用，自动切到阿里云 TTS（Key 与 ASR 共用）');
        provider = 'aliyun';
      } else if (xinyunKey) {
        console.log('[TTS] piper.exe 不可用，自动切到芯云 TTS');
        provider = 'xinyun';
      }
    }
  }

  // 默认 provider：优先用户保存的 > 自动检测
  return {
    provider,
    voiceId:        stored.ttsVoiceId   || 'nova',
    xinyunKey:      config.apiKey || '',
    aliyunKey:      stored.aliyunKey || voiceStored.aliyunApiKey || '',  // 与 ASR 共用阿里云 Key
    piperExe:       stored.piperExe || '',
    piperModelDir:  stored.piperModelDir || '',
