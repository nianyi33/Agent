...
export function startAPI(port = 3721, { getStateSnapshot = null, onActivated = null } = {}) {
  ...
  const server = http.createServer(async (req, res) => {
    ...
    if (req.method === 'GET' && url.pathname === '/agent-profile') {
      ...
      jsonResponse(res, 200, { name: getAgentName(), provider: status.provider, model: status.model })
      return
    }
    ...
    ws.on('message', (raw) => {
      ...
      if (!configured) {
        try {
          ...
          // If frontend sent an API key, merge it in — takes priority over config.json
          const cfg = { ...rawCfg, ...(msg.apiKey ? { aliyunApiKey: msg.apiKey } : {}) };
          session = createCloudASRSession(
            { provider, lang: msg.lang || 'zh', ...cfg },
            (text, isFinal, seg) => {
            ...
            () => { try { ws.send(JSON.stringify({ type: 'error', message: '语音服务连接已断开，请重试' })) } catch {} },
