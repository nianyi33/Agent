...
// wechat-ilink-client 是可选原生依赖，未安装时优雅跳过
let _ilink = null
let _ilinkSymbols = null  // { WeChatClient, MessageType, MessageItemType, ... }
async function _loadIlink() {
  if (_ilink) return _ilink
  try {
    _ilink = await import('wechat-ilink-client')
    _ilinkSymbols = {
      WeChatClient: _ilink.WeChatClient,
      MessageType: _ilink.MessageType,
      MessageItemType: _ilink.MessageItemType,
      UploadMediaType: _ilink.UploadMediaType,
      aesEcbPaddedSize: _ilink.aesEcbPaddedSize,
      decryptAesEcb: _ilink.decryptAesEcb,
      encryptAesEcb: _ilink.encryptAesEcb,
      getMimeFromFilename: _ilink.getMimeFromFilename,
    }
    return _ilink
  }
  catch (e) { console.warn('[WeChat] wechat-ilink-client 未安装，微信桥接不可用'); return null }
}
function _getSyms() { if (!_ilinkSymbols) throw new Error('wechat-ilink-client not loaded'); return _ilinkSymbols }
...
function mediaRefForItem(item) {
  switch (itemType(item)) {
    case _getSyms().MessageItemType.IMAGE: {
    ...
    case _getSyms().MessageItemType.VOICE:
...
function inferInboundMediaExt({ downloaded, item, fileName }) {
  ...
  if (itemType(item) === _getSyms().MessageItemType.VOICE) return defaultExtForVoice(item)
...
async function downloadClawbotMediaItemFallback(item, cdnBaseUrl) {
  ...
  const data = ref.aesKeyBase64
    ? _getSyms().decryptAesEcb(encrypted, parseCdnAesKey(ref.aesKeyBase64))
    : encrypted
...
export function pickClawbotInboundMediaItems(msg) {
  ...
  const main = items
    .filter(item => isClawbotMediaItem(item)
      ...
      && !(itemType(item) === _getSyms().MessageItemType.VOICE && item?.voice_item?.text))
  ...
  const refMedia = items.find(item =>
    itemType(item) === _getSyms().MessageItemType.TEXT
...
function extractClawbotText(msg) {
  const text = _getSyms().WeChatClient.extractText?.(msg)
...
export async function handleClawbotInboundMessage(msg, {
  ...
  if (!msg) return null
  if (msg.message_type != null && Number(msg.message_type) !== _getSyms().MessageType.USER) return null
...
function inferUploadMediaType(filePath) {
  const s = _getSyms()
  const mime = s.getMimeFromFilename(filePath)
  if (mime.startsWith('image/')) return { mediaType: s.UploadMediaType.IMAGE, kind: 'image' }
  if (mime.startsWith('video/')) return { mediaType: s.UploadMediaType.VIDEO, kind: 'video' }
  return { mediaType: s.UploadMediaType.FILE, kind: 'file' }
}
...
async function uploadEncryptedBuffer(uploadUrl, plaintext, aeskey) {
  const ciphertext = _getSyms().encryptAesEcb(plaintext, aeskey)
  let lastError = null
...
async function uploadMediaViaClawbotApi(userId, filePath) {
  ...
  const filesize = _getSyms().aesEcbPaddedSize(rawsize)
