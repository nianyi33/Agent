// TreeSloth自知识文档 —— 解释自身的代码机制、架构与界面设计。
...
export const SELF_KNOWLEDGE_TOPICS = {
  self_architecture: {
    ...
    title: 'TreeSloth架构与运行机制',
    subtitle: 'How BaiLongma Works',
    icon: '⚙',
    summary: `TreeSloth（BaiLongma）是一套 Electron + Node.js 的"持续意识"框架，当前版本 ${APP_VERSION}。它不是被动等待提问的聊天机器人，而是一个持续运行、自主感知、带长期记忆的 Agent。以下是当前版本的完整机制说明。`,
    sections: [
      {
        title: '整体架构',
        content: `TreeSloth由三层构成：
...
■ Brain UI 前端（src/ui/brain-ui/）
  ...
  - 详见"TreeSloth界面设计"文档主题（ui_design）

数据落在 SQLite（src/db.js）与 data/ 目录；运行配置在 config.json + 若干独立配置文件。`,
      },
      {
        title: '意识循环：L1 / L2 两种入口',
        content: `TreeSloth不是"两个人格"，而是同一个 AI 的两种触发入口，共享同等的上下文质量（记忆、人物卡、思维、UI 状态）：
...
配套的"成果审视分身"会在收尾前复查（见下）。`,
      ...
      {
        title: '动态记忆池（核心机制）',
        content: `TreeSloth的记忆不是简单的"存一段查一段"，而是一套"一切皆记忆 / 少即是强"的动态池——目标是每轮注入"合适的上下文"，不是"召回越多越好"。
...
Key 配置：serper / brave / tavily / jina / searxng，存在 config.json 顶级字段或环境变量。`,
      ...
      {
        title: '上下文感知：环境采集',
        content: `TreeSloth持续感知运行环境，结果进"补充上下文"：
...
■ 取证与可观测（排障用）：
  ...
  ui_design: {
    ...
    icon: '🖥',
    summary: 'TreeSloth的界面叫 Brain UI，运行在 Electron 渲染进程。Agent 通过声明式 Scene 协议驱动界面（UI = f(scene)），并能感知界面状态。以下是界面各部分的设计说明。',
    sections: [
      {
        title: 'Brain UI 总览',
...
原则：仅当"可视化比纯文字更清楚"时才 ui_set，普通问答不主动投影。`,
  ...
  - 托盘后台运行，不抢主窗口焦点`,
      },
      {
        ...
        content: `TreeSloth界面有一套统一的视觉规范，新增 UI 必须遵循：
  - 纯文本流，无卡片包裹、无滚动条
...
export function detectSelfKnowledgeTopic(text) {
  ...
  // 架构 / 运行机制相关
  if (
    /(你的代码|你.*怎么运行|你.*怎么工作|你.*架构|你.*如何运作|TreeSloth.*代码|treesloth-ai-agent.*代码|你.*实现|代码机制|运行机制|技术架构|你.*内部|你.*系统|你.*模块|你.*是怎么|你.*如何思考|你.*心跳|意识循环|认知循环|动态记忆|记忆池|审视分身|ticker|queue\.js|control\.js|llm\.js|prompt\.js|memory.*机制|记忆.*(系统|机制)|工具.*调用|capability|executor|l1.*l2|l2.*l1|两个入口|react.*任务|self.?knowledge|自知识|自我感知)/.test(
      t
    )
  ) {
