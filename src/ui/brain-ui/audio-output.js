// audio-output.js — TreeSloth语音输出设备路由
//
// 背景 / 为什么需要这个模块：
//   TreeSloth是"会说话"的助手，TTS 默认走 `new Audio()`，也就是听天由命跟着系统默认播放设备走。
//   而大众用户的机器上经常装着虚拟声卡（Steam 串流 / NVIDIA Virtual / VB-Audio / Pico VR 等），
//   或者拔掉耳机后 Windows 没有正确回退到内置扬声器 —— 系统默认就卡在一个"哑巴"虚拟设备上。
//   结果：TreeSloth在说话，但用户什么都听不到，而且双方都不知道为什么。对语音产品这是致命的静默失败。
//
...
const OUTPUT_DEVICE_KEY = 'treesloth-ai-agent-voice-output-device-id'; // '' = 自动；否则为具体 deviceId
const BANNER_DISMISS_KEY = 'treesloth-ai-agent-voice-output-banner-dismissed'; // 用户主动忽略后本会话不再弹
...
// 关键：TreeSloth TTS 只要 AudioContext 在运行，就会经 createMediaElementSource 走 Web Audio，
