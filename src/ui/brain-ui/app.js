...
import { initHoldButton } from "./voice-hold-button.js";
...
const ACTIVATION_WARMUP_KEY = "treesloth-ai-agent_activation_warmup_until";
const UI_ZOOM_STORAGE_KEY = "treesloth-ai-agent_ui_zoom_factor";
const MAX_CHAT_HISTORY = 60;
const DEFAULT_AGENT_NAME = "TreeSloth";
const DEFAULT_UI_ZOOM = 1.1;
const MIN_UI_ZOOM = 0.8;
const MAX_UI_ZOOM = 1.8;
const UI_ZOOM_STEP = 0.1;
const UI_ZOOM_WHEEL_STEP = 0.05;
const MEMORY_GRAPH_STORAGE_KEY = "treesloth-ai-agent-memory-graph-enabled";
...
const IGNORED_VERSION_KEY = "treesloth-ai-agent_ignored_update_version";
const SUPPRESS_UPDATES_KEY = "treesloth-ai-agent_suppress_update_notifications";
...
async function loadAgentProfile() {
  try {
    ...
    const el = document.getElementById("ctx-llm-model");
...
(function initTheme() {
  ...
  if (!saved || saved === "midnight" || saved === "phosphor" || saved === "violet" || saved === "rose" || saved === "arctic" || saved === "sand") saved = "dark";
  applyTheme(saved);
})();
  ...
  switch (type) {
    ...
    case "message":
      if (data.from === "consciousness") {
        ...
        // 🎙️ 语音对话模式：自动朗读所有 AI 回复
        if (window._voiceModeActive && !liveTurnSpeak && !shouldSpeakMessage) {
          playTTSReply(toPlainSpeech(data.content));
        }
        liveReplyActive = false;
        liveRawText = "";
        liveTurnSpeak = false;
    ...
    case "agent_name_updated":
      ...
      break;
    case "media_mode":
      window.dispatchEvent(new CustomEvent("treesloth-ai-agent:media", { detail: data }));
      break;
    case "aivideo_mode":
      window.dispatchEvent(new CustomEvent("treesloth-ai-agent:aivideo", { detail: data }));
      break;
    case "hotspot_mode":
    ...
    case "person_card_mode":
      ...
      break;
    case "social_status":
      window.dispatchEvent(new CustomEvent("treesloth-ai-agent:social_status", { detail: data }));
      break;
...
const TTS_STREAMING_KEY = 'treesloth-ai-agent.tts.streaming';
...
async function playTTSReply(text) {
  ...
  try {
    ...
    // 流式播放：仅当支持 MSE 且是 MPEG 流时走 MediaSource。WAV(Piper) 直接 blob 播放
    const contentType = resp.headers.get('Content-Type') || ''
    const isMpegStream = contentType.includes('audio/mpeg')
    if (ttsCanStream() && isMpegStream && resp.body) {
    ...
    console.warn('[TTS] playTTSReply failed:', err?.message || err);
    clearTTSAudioGraph();
    ttsCurrentText = '';
...
function toPlainSpeech(md) {
  return String(md || '')
    .replace(MARKER_STRIP_RE, '')  // 先踢掉 [RECALL:...] 等内部标记
    .replace(/\*\*(.+?)\*\*/g, '$1')
    .replace(/\*(.+?)\*/g, '$1')
    .replace(/`(.+?)`/g, '$1')
    .replace(/#{1,6}\s+/g, '')
...
initWechatPopup();
initFeishuPopup();
initHotspot();       // 绑定热点面板关闭按钮 + 渲染内容
initWorldcup();       // 绑定世界杯面板关闭按钮
...
function initTTSSettings() {
  ...
  const credSections = {
    'piper-local': null,  // Piper needs no credentials
    xinyun: document.getElementById("tts-creds-xinyun"),
  };
  ...
  function showCredSection(provider) {
    ...
    // Qwen3-TTS 面板：检查状态
    if (provider === 'qwen3-local') checkQwen3TTSStatus()
  }
  ...
  fetch(`${API}/settings/tts`).then(r => r.json()).then(({ tts, voices, providers }) => {
    // Populate provider dropdown (never populated before → was always empty!)
    if (providers && providerSel) {
      providerSel.innerHTML = providers.map(p =>
        `<option value="${p.id}">${p.label}</option>`
      ).join("");
    }
    if (voices) allVoices = voices;
    const provider = tts?.ttsProvider || "piper-local";
    if (tts?.ttsProvider) providerSel.value = tts.ttsProvider;
    else providerSel.value = "piper-local";
  ...
  const ttsSaveBtn = document.getElementById("settings-save-tts");
  if (ttsSaveBtn) {
    ttsSaveBtn.addEventListener("click", () => {
      ...
      fetch(`${API}/settings/tts`, {
        ...
        if (voiceFeedback) { voiceFeedback.textContent = "已保存"; voiceFeedback.className = "settings-feedback"; setTimeout(() => { voiceFeedback.textContent = ""; }, 3000); }
      }).catch(() => {
        const voiceFeedback = document.getElementById("settings-voice-feedback");
        if (voiceFeedback) { voiceFeedback.textContent = "保存失败"; voiceFeedback.className = "settings-feedback error"; setTimeout(() => { voiceFeedback.textContent = ""; voiceFeedback.className = "settings-feedback"; }, 3000); }
      });
    });
  }
...
(function initSettings() {
  ...
  const voiceFeedback   = document.getElementById("settings-asr-feedback");
  ...
  async function loadSettings() {
    try {
      ...
      if (modelSelect && llm.model) modelSelect.value = llm.model;
      if (llmKeyInput) llmKeyInput.value = llm.apiKey || "";
  ...
  const VOICE_LANG_KEY       = "treesloth-ai-agent-voice-lang";
  const VOICE_AUTO_SEND_KEY  = "treesloth-ai-agent-voice-auto-send";
  const VOICE_AUTO_MIC_KEY   = "treesloth-ai-agent-voice-auto-mic";
  const VOICE_THRESHOLD_KEY  = "treesloth-ai-agent-voice-threshold";
  const VOICE_PROVIDER_KEY   = "treesloth-ai-agent-voice-provider";
  const VOICE_MIC_DEVICE_KEY = "treesloth-ai-agent-voice-mic-device-id";

  function applyVoiceProviderUI(provider) {
    // Show credential fields only for providers that need API keys.
    const credEl = document.getElementById("voice-cred-aliyun");
    if (credEl) credEl.style.display = (provider === 'aliyun' || provider === 'xinyun') ? "" : "none";
  }
  ...
  async function loadVoiceSettings() {
    ...
    // 以 localStorage 保存的值为准，不以后端 config.json 覆盖
    const savedProvider = localStorage.getItem(VOICE_PROVIDER_KEY) || "whisper-cpp";
    ...
    const statusBtn = document.getElementById("voice-install-status-btn")
    if (installBtn) installBtn.onclick = async () => {
      const s = document.getElementById("voice-python-status")
      installBtn.disabled = true; installBtn.textContent = "安装中…"
      try {
        const r = await fetch("http://127.0.0.1:3721/voice/install", { method: "POST" })
        ...
        while (true) {
          ...
          for (const line of buf.split('\n')) {
            ...
            try {
              ...
              const msg = (d.progress ? d.progress + '% ' : '') + (d.message || d.error || '')
              if (s) s.textContent = (d.step === 'error' ? '❌ ' : '') + msg
              if (d.step === 'error') { installBtn.disabled = false; installBtn.textContent = "⚡ 一键安装" }
              if (d.step === 'done') { installBtn.textContent = "✅ 已安装" }
            } catch {}
          }
          buf = ''
        }
      } catch { if (s) s.textContent = "❌ 后端未启动"; installBtn.disabled = false; installBtn.textContent = "⚡ 一键安装" }
    }
    if (statusBtn) statusBtn.onclick = async () => {
      const s = document.getElementById("voice-python-status")
      if (s) s.textContent = "检测中…"
      try {
        const r = await fetch("http://127.0.0.1:3721/voice/status")
        const d = await r.json().catch(() => ({}))
        if (s) s.textContent = d.status === "running" ? "✅ FunASR 已就绪" : (d.message || d.status || "未检测")
      } catch { if (s) s.textContent = "❌ 后端未启动" }
    }
  }
  ...
  if (saveVoiceBtn) {
    saveVoiceBtn.addEventListener("click", async () => {
      ...
      const provider  = voiceProviderSelect?.value || "whisper-cpp";
      ...
      window.dispatchEvent(new CustomEvent("treesloth-ai-agent:voice-threshold", { detail: { threshold } }));
  ...
  if (providerSelect) {
    // provider select removed — no longer in new UI
  }

  if (modelSelect) {
    // model select change — syncOfficialCustomModelRow removed (no longer in new UI)
  }
  ...
  saveLlmBtn?.addEventListener("click", async () => {
    ...
    if (!apiKey) {
      showFeedback(llmFeedback, "请填入 API Key", true);
      return;
    }
    saveLlmBtn.disabled = true;
    try {
      const body = { provider: "xinyun" };
      body.apiKey = apiKey;
  ...
  // Model save handler (new UI — direct model selection)
  const saveModelBtn = document.getElementById("settings-save-model");
  const modelFeedback = document.getElementById("settings-model-feedback");
  saveModelBtn?.addEventListener("click", async () => {
    const model = modelSelect?.value;
    if (!model) {
      showFeedback(modelFeedback, "请选择一个模型", true);
      return;
    }
    saveModelBtn.disabled = true;
    try {
      const body = { provider: "xinyun", model };
      const apiKey = llmKeyInput?.value?.trim();
      ...
      const data = await res.json();
      if (data.ok) {
        showFeedback(modelFeedback, "已保存");
        const el = document.getElementById("ctx-llm-model");
        if (el && model) el.textContent = model;
        cachedLlm = data;
      } else {
        showFeedback(modelFeedback, data.error || "保存失败", true);
      }
    } catch { showFeedback(modelFeedback, "请求失败", true); }
  ...
  window.addEventListener("treesloth-ai-agent:social_status", (e) => {
    const d = e.detail;
  ...
  async function loadUpdateSettings() {
    ...
    bridge.getVersion?.().then(v => {
      if (settingsCurrentVersion && v) settingsCurrentVersion.textContent = v
    }).catch(() => {})
    ...
    removeUpdaterListener = bridge.onUpdaterStatus?.((payload = {}) => {
      ...
      switch (stage) {
        ...
        case "downloading":
          ...
          break
        case "downloaded":
          setUpdateStatusText(`版本 ${ver} 已就绪`, "ready")
        ...
        case "up-to-date":
          ...
          break
        case "error":
          setUpdateStatusText(`更新检查失败`, "error")
  ...
  let giteeUpdateUrl = null

  settingsCheckUpdateBtn?.addEventListener("click", async () => {
    if (!window.Bailongma?.isElectron) return
    setUpdateStatusText("正在检查更新…", "checking")
    showUpdateButtons({ checkDisabled: true, checkLabel: "检查中…" })
    try {
      const info = await window.Bailongma.checkForUpdate?.()
      // 结果由 onUpdaterStatus 事件驱动，这里不重复处理
      if (!info) {
        setUpdateStatusText("检查失败，请稍后再试", "error")
        showUpdateButtons({ checkLabel: "重试" })
      }
    } catch (err) {
      setUpdateStatusText(`检查失败：${err?.message || "网络错误"}`, "error")
      showUpdateButtons({ checkLabel: "重试" })
    }
  })
  ...
  settingsDownloadUpdateBtn?.addEventListener("click", async () => {
    ...
    showUpdateButtons({ check: false })
    try {
      await window.Bailongma.downloadUpdate?.()
    } catch (err) {
  ...
  settingsInstallUpdateBtn?.addEventListener("click", () => {
    window.Bailongma?.installUpdate?.()
  })
...
initVoicePanel({
  ...
  chatInput:  document.getElementById("msg-input"),
  getSendBtn:    () => document.getElementById("send-btn"),
  getSendMessage: (options) => chat?.send?.(options),
  getLang:       () => localStorage.getItem("treesloth-ai-agent-voice-lang") || "zh-CN",
  getAutoSend:   () => localStorage.getItem("treesloth-ai-agent-voice-auto-send") !== "false",
  getAutoMic:    () => localStorage.getItem("treesloth-ai-agent-voice-auto-mic") === "true",
});

// ── 微信式按住说话按钮 ──
initHoldButton({ buttonId: 'voice-hold-btn' });
...
(function initSpeakToggle() {
  ...
  // 从 localStorage 恢复状态
  const stored = localStorage.getItem('treesloth-ai-agent-tts-speak-mode');
  window._voiceModeActive = stored === 'true';

  function updateUI() {
    if (window._voiceModeActive) {
      btn.textContent = '🔊';
      btn.classList.add('on');
      btn.title = '朗读模式：开（点击关闭）';
    } else {
      btn.textContent = '🔇';
      btn.classList.remove('on');
      btn.title = '朗读模式：关（点击开启）';
    }
  }
  updateUI();

  btn.addEventListener('click', () => {
    window._voiceModeActive = !window._voiceModeActive;
    localStorage.setItem('treesloth-ai-agent-tts-speak-mode', String(window._voiceModeActive));
    updateUI();
  });
})();
...
(function initThemeToggle() {
  ...
  function updateUI() {
    ...
    btn.title = isLight ? '浅色模式（点击切换深色）' : '深色模式（点击切换浅色）';
  }
  updateUI();
  ...
  btn.addEventListener('click', () => {
    ...
    localStorage.setItem('jarvis-brain-ui-theme', isLight ? 'dark' : 'light');
    updateUI();
  });
})();
...
(function initAIVideoMode(){
  ...
  function openPanel(configured){
    ...
    if(configured===false){ composeErr.textContent="尚未配置火山方舟（Seedance）API Key —— 把 key 发给TreeSloth即可（例如「火山视频 你的APIKey」），配置后就能在这里生成。"; composeErr.hidden=false; }
  ...
  window.addEventListener("treesloth-ai-agent:aivideo", function(e){ handle(e.detail||{}); });
...
// ── LLM 连接状态 ping 检测 ──
async function pingXinyun() {
  const dot = document.getElementById('settings-ping-dot')
  const text = document.getElementById('settings-ping-text')
  const value = document.getElementById('settings-ping-value')
  if (!dot || !text || !value) return
  ...
  try {
    ...
    const key = (keyInput && keyInput.value.trim()) || 'none'
    const resp = await fetch('https://xinyuntoken.com/v1/models', {
      method: 'GET',
    ...
    if (elapsed < 100) {
      dot.style.background = '#77df9f'; text.textContent = '优秀'
    } else if (elapsed < 300) {
      dot.style.background = '#efb667'; text.textContent = '良好'
    } else if (elapsed < 800) {
      dot.style.background = '#f0a050'; text.textContent = '一般'
    } else {
      dot.style.background = '#ff7f8a'; text.textContent = '较慢'
    }
  } catch {
    dot.style.background = '#555'
    value.textContent = '—'
...
// Bind ping button and auto-trigger
const _pingInit = () => {
  setTimeout(pingXinyun, 800)
  const btn = document.getElementById('settings-ping-btn')
  if (btn) btn.addEventListener('click', pingXinyun)
}
if (document.readyState === 'loading') {
  document.addEventListener('DOMContentLoaded', _pingInit)
} else {
  _pingInit()
}

// 隐藏启动加载动画：等 Brain UI 完成首帧渲染后再淡出，避免黑屏
function hideAppLoading() {
  ...
  requestAnimationFrame(() => {
    requestAnimationFrame(() => {
      ...
      // CSS transition 0.35s 后移除 DOM
      setTimeout(() => { try { loader.remove() } catch {} }, 400)
    })
  })
}
if (document.readyState === 'loading') {
...
// ── v1.0 背景 Canvas（氛围光晕 + 微粒子） ──
(function initBackgroundCanvases() {
  const ac = document.getElementById('ambientCanvas');
  if (ac) {
    const actx = ac.getContext('2d');
    let w, h, t = 0;
    function resizeA() { w = ac.width = window.innerWidth; h = ac.height = window.innerHeight; }
    resizeA(); window.addEventListener('resize', resizeA);
    function renderAmbient() {
      actx.clearRect(0, 0, w, h); actx.globalCompositeOperation = 'screen'; t += 0.003;
      const blobs = [{cx:w*0.3,cy:h*0.3,r:340,c:'rgba(146,92,245,0.06)'},{cx:w*0.7,cy:h*0.6,r:280,c:'rgba(79,216,255,0.04)'},{cx:w*0.5,cy:h*0.5,r:400,c:'rgba(255,180,84,0.03)'}];
      blobs.forEach((b,i)=>{const ox=Math.sin(t*0.4+i)*40,oy=Math.cos(t*0.35+i)*30,s=1+Math.sin(t*0.6+i)*0.15;actx.beginPath();const g=actx.createRadialGradient(b.cx+ox,b.cy+oy,0,b.cx+ox,b.cy+oy,b.r*s);g.addColorStop(0,b.c);g.addColorStop(0.5,b.c.replace('0.06','0.015').replace('0.04','0.01').replace('0.03','0.008'));g.addColorStop(1,'transparent');actx.fillStyle=g;actx.arc(b.cx+ox,b.cy+oy,b.r*s,0,Math.PI*2);actx.fill();});
      actx.globalCompositeOperation='source-over';requestAnimationFrame(renderAmbient);
    }renderAmbient();
  }
  const mc = document.getElementById('microCanvas');
  if (mc) {
    const mctx = mc.getContext('2d');let mw,mh,pts=[],mt=0;function resizeM(){mw=mc.width=window.innerWidth;mh=mc.height=window.innerHeight;}resizeM();window.addEventListener('resize',resizeM);
    const GA=Math.PI*(3-Math.sqrt(5));for(let i=0;i<200;i++){const y=1-(2*i)/199,r=Math.sqrt(1-y*y)*0.9,th=i*GA;pts.push({x:r*Math.cos(th),y:r*Math.sin(th),z:y});}
    function renderMicro(){mctx.clearRect(0,0,mw,mh);mt+=0.002;const cx=mw*0.82,cy=mh*0.35,radius=260;pts.forEach((p,i)=>{const px=cx+p.x*radius+Math.sin(mt*1.3+i*0.1)*4,py=cy+p.z*radius+Math.cos(mt*0.9+i*0.1)*4,alpha=0.06+Math.sin(mt*2+i*0.3)*0.03;mctx.beginPath();mctx.arc(px,py,0.5+Math.sin(mt*1.5+i*0.2)*0.3,0,Math.PI*2);mctx.fillStyle='rgba(146,92,245,'+alpha+')';mctx.fill();});requestAnimationFrame(renderMicro);}renderMicro();
  }
})();
...
(function initNavBar() {
  ...
  document.getElementById('nav-voice-btn')?.addEventListener('click',()=>voiceBtn?.click());
  document.getElementById('nav-settings-btn')?.addEventListener('click',()=>settingsBtn?.click());
  document.getElementById('nav-theme-btn')?.addEventListener('click',()=>themeBtn?.click());
  document.getElementById('nav-memory-btn')?.addEventListener('click',()=>{const g=document.getElementById('graph');if(g)g.style.display=g.style.display==='none'?'':'none';});
  document.getElementById('nav-hotspot-btn')?.addEventListener('click',()=>{if(typeof toggleHotspot==='function')toggleHotspot();});
  document.querySelectorAll('.nav-item').forEach(item=>{item.addEventListener('click',()=>{document.querySelectorAll('.nav-item').forEach(i=>i.classList.remove('sel'));item.classList.add('sel');});});
})();

// ── Qwen3-TTS 本地面板 ──
const GITHUB_TTS_EXE_URL = 'https://github.com/nianyi33/Agent/releases/download/tts/tts-server.exe'

async function checkQwen3TTSStatus() {
  const statusEl = document.getElementById('tts-qwen3-status')
  const dlBtn = document.getElementById('tts-qwen3-dl-btn')
  if (!statusEl) return
  try {
    const resp = await fetch('http://127.0.0.1:3721/tts/status')
    const data = await resp.json().catch(() => ({}))
    if (data.status === 'running') {
      statusEl.textContent = '🟢 运行中 (port ' + (data.port||8800) + ')'
      if (dlBtn) dlBtn.style.display = 'none'
    } else if (data.status === 'starting') {
      statusEl.textContent = '🟡 正在启动…'
      if (dlBtn) dlBtn.style.display = 'none'
    } else if (data.status === 'error' && data.message.includes('找不到')) {
      statusEl.textContent = '⚪ 服务未安装 — 点下方下载'
      if (dlBtn) dlBtn.style.display = ''
    } else {
      statusEl.textContent = '⚪ ' + (data.message || data.status || '未启动')
      if (dlBtn) dlBtn.style.display = ''
    }
  } catch {
    statusEl.textContent = '⚪ 服务未安装 — 点下方下载'
...
document.getElementById('tts-qwen3-start-btn')?.addEventListener('click', async () => {
  const statusEl = document.getElementById('tts-qwen3-status')
  const btn = document.getElementById('tts-qwen3-start-btn')
  if (btn) { btn.disabled = true; btn.textContent = '启动中…' }
  if (statusEl) statusEl.textContent = '🟡 正在启动（首次需下载模型 ~2GB）…'
  try {
    const resp = await fetch('http://127.0.0.1:3721/tts/start', { method: 'POST',
      headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ model: '0.6B' }) })
    const data = await resp.json().catch(() => ({}))
    if (resp.ok) {
      setTimeout(checkQwen3TTSStatus, 3000)
      setTimeout(checkQwen3TTSStatus, 8000)
    } else {
      if (statusEl) statusEl.textContent = '❌ 启动失败 — ' + (data.message || data.error || '请检查 Python/qwen-tts 安装')
    }
  } catch {
    if (statusEl) statusEl.textContent = '❌ 无法连接后端服务'
  }
  if (btn) { btn.disabled = false; btn.textContent = '启动服务' }
})

document.getElementById('tts-qwen3-stop-btn')?.addEventListener('click', async () => {
  const statusEl = document.getElementById('tts-qwen3-status')
  try {
    await fetch('http://127.0.0.1:3721/tts/stop', { method: 'POST' })
    if (statusEl) statusEl.textContent = '⚪ 已停止'
    setTimeout(checkQwen3TTSStatus, 500)
  } catch {
    if (statusEl) statusEl.textContent = '状态：未知'
  }
})