...
export function createContinuousPolicy(core, { getAutoSend, chatInput }) {
  // ─── 自动发送状态 ───
  ...
  //   - 自动发送开启：静默后自动发送消息
  //   - 自动发送关闭：什么都不做（文字已由 applyTranscript 实时写入输入框，用户手动发送）
  function scheduleAutoSend() {
    if (core.pttHolding) return;
    noteTranscriptActivity();
    if (getAutoSend?.() === false) return; // 关闭自动发送 → 文字已在输入框，无需额外动作
    if (autoSendTimer) return;
    const tick = () => {
      if (core.pttHolding) { autoSendTimer = null; return; }
