...
// 点云算法移植自 ACUI (Remix)/Voice Component.html
	...
	import { createBrowserASRSession, isBrowserASRAvailable, setBrowserASRLang } from './browser-voice.js';

// whisper-cpp 在后端运行，前端通过 HTTP 检查可用性
async function isWhisperCPPAvailable() {
  try {
    const resp = await fetch('http://127.0.0.1:3721/voice/whisper-cpp-status')
...
export function createVoiceCore({ canvas, transcript, chatInput, getSendMessage, getLang }) {
  ...
  let browserASRSession = null;    // 浏览器本地语音识别会话
  ...
  function applyTranscript(msg) {
    ...
    if (msg.is_final) {
      ...
      if (transcript) transcript.textContent = accumulatedText;
      // 立即写入聊天输入框，让用户看到实时识别结果
      if (changed) fillChatInput(accumulatedText);
    ...
    // interim 也实时写入输入框，让用户看到"正在识别"的文字
    fillChatInput(lastTranscriptText);
    return false;
  }

  // 将识别文本写入聊天输入框
  let _lastInputFill = '';
  function fillChatInput(t) {
    if (!t || t === _lastInputFill || !chatInput) return;
    _lastInputFill = t;
    chatInput.value = t;
  ...
  // ─── 语音识别结果发送（自动发送/PTT 松手时调用；文字已由 applyTranscript 实时写入输入框） ───
  function sendRecognizedVoiceText() {
    if (!lastTranscriptText) return;
    const text = lastTranscriptText;
    // 发送消息到对话 — channel='语音识别' 让后端识别这是语音轮，
    // 从而自动调用 autoSpeakForVoiceReply → TTS 朗读回复
    getSendMessage?.({ text, channel: '语音识别' });
  ...
  function handleAsrMessage(msg) {
    if (msg.type === 'transcript') {
      ...
      if (Date.now() < transcriptSuppressUntil) {
        // Only suppress if within PTT tail window AND no new speech has started
        // (new loud audio means user started speaking again immediately)
        const silenceGap = Date.now() - (lastLoudTs || 0);
        if (silenceGap < 600) {
          transcriptSuppressUntil = 0; // new speech detected, cancel suppress
        } else {
          resetTranscriptAccumulation();
          lastTranscriptText = '';
          if (transcript) transcript.textContent = '';
  ...
  function connectCloudWs() {
    ...
    ws.onopen = () => {
      ...
      let provider = localStorage.getItem(VOICE_PROVIDER_KEY) || 'whisper-cpp';
      // whisper-cpp 映射到已有本地 Whisper Python 管道 (ws://127.0.0.1:3723)
      if (provider === 'whisper-cpp') provider = 'funasr';
  ...
  async function startCloudStream(stream) {
    const provider = localStorage.getItem(VOICE_PROVIDER_KEY) || 'whisper-cpp'

    // whisper-cpp → 走 AudioWorklet → WebSocket → 本地 Whisper 服务
    // browser → Chrome SpeechRecognition API
    if (provider === 'browser') {
      if (!isBrowserASRAvailable()) {
        setStatus('error')
        onError?.('浏览器不支持语音识别，请用 Chrome/Edge')
        return
      }
      const lang = getLang?.()?.split('-')[0] || 'zh'
      setBrowserASRLang(lang)
      browserASRSession = createBrowserASRSession(
        lang,
        (text, isFinal, seg) => {
          setStatus('recognizing')
          handleAsrMessage({ text, isFinal, seg })
        },
        (err) => { setStatus('error'); onError?.(err) },
        () => {
          if (cloudWsIntentional) return
          browserASRSession = null
          if (transcript) transcript.textContent = ''
        }
      )
      resetTranscriptAccumulation()
    ...
    diagStart();
    startWatchdog();
    connectCloudWs();  // whisper-cpp 也走 WS — connectCloudWs 会自动映射到本地 Whisper
  }
  ...
  function stopCloudStream({ preserveProcessor = false, flushDelayMs = 5000 } = {}) {
    if (browserASRSession) { browserASRSession.stop(); browserASRSession = null; }
    cloudWsIntentional = true; // 标记为主动关闭，防止 onclose 触发重连
    const ws = cloudWs;
    try {
      if (ws && ws.readyState === WebSocket.OPEN) {
        ws.send(JSON.stringify({ type: 'flush' }));
        // 等待足够长让 Python Whisper 完成转写（base 模型 CPU 上约 1-2s，给 5s 余量）
        setTimeout(() => {
          try { ws.close(); } catch {}
  ...
  async function startSession() {
    ...
    // 浏览器本地识别：Speech API 自己管麦克风，不占硬件锁
    if (provider === 'browser') {
      if (!isBrowserASRAvailable()) {
        micActive = false; userWantedMic = false; syncState();
        setStatus('error');
        // 显示可见错误提示给用户
        if (typeof transcript !== 'undefined' && transcript) {
          transcript.textContent = '⚠️ 浏览器语音识别仅支持 Chrome/Edge。桌面端请使用云端 ASR（阿里云/火山等）'
      ...
      browserASRSession = createBrowserASRSession(
        lang,
        (text, isFinal, seg) => { setStatus('recognizing'); handleAsrMessage({ text, isFinal, seg }); },
        (err) => { setStatus('error'); },
        () => { cloudWsIntentional = false; browserASRSession = null; }
      );
      setStatus('listening');
      return { browserMode: true };
    }
  ...
  function stopSession({ keepIntent = false } = {}) {
    ...
    // 发 flush → 获取 Python Whisper 对缓冲音频的最终转写结果
    // stopCloudStream 内部关闭 WS 前会等 5s（flushDelayMs），给 Whisper 留足转写时间
    stopCloudStream({ flushDelayMs: 5000 });
    ...
    // 轮询等待 flush 后的转录结果（Whisper 转写需 1-2s）
    // 转录到达时 handleAsrMessage → applyTranscript → lastTranscriptText 被更新
    ...
    const poll = setInterval(() => {
      if (lastTranscriptText) {
        ...
        fillChatInput(lastTranscriptText);
        finalize();
      } else if (Date.now() - flushStart >= 5000) {
        clearInterval(poll);
        finalize();
      }
    }, 100);
  ...
  function suspendForTTS() {
    ...
    stopCloudStream({ preserveProcessor: true, flushDelayMs: 200 }); // TTS 挂起只需快速清理，不等 flush 响应
    setStatus('speaking');
  }
  ...
  async function resumeSession(fromBargein = false) {
    ...
    if (micActive && micData && (cloudWorkletNode || cloudProcessor)) {
      ...
      bargeinWs.onopen = () => {
        ...
        let provider = localStorage.getItem(VOICE_PROVIDER_KEY) || 'whisper-cpp';
        if (provider === 'whisper-cpp') provider = 'funasr';  // remap for barge-in path
        const lang = getLang?.()?.split('-')[0] || 'zh';
