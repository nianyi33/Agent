...
import { getVoiceStatus, startVoiceServer } from './manager.js'
...
export function createCloudASRSession(config, onTranscript, onError, onClose, onEvent) {
  let { provider = 'aliyun', lang = 'zh' } = config

  // 兼容前端传来的 whisper-cpp 选择 → 映射到本地 Whisper 管道
  if (provider === 'whisper-cpp') provider = 'funasr'

  if (provider === 'local') {
    if (process.platform === 'darwin') return createMacSpeechSession({ ...config, lang }, onTranscript, onError, onClose)
    // Windows/Linux → whisper_server.py WebSocket (ws://127.0.0.1:3723)
    return createLocalWhisperSession(lang, onTranscript, onError, onClose)
  }

  if (provider === 'funasr') {
    return createFunASRSession(lang, onTranscript, onError, onClose)
  }
...
// ─── 本地 Whisper 服务（whisper_server.py → ws://127.0.0.1:3723） ───
function createLocalWhisperSession(lang, onTranscript, onError, onClose) {
  const WHISPER_WS_URL = 'ws://127.0.0.1:3723'
  let ws = null
  let ready = false
  let closed = false
  const pending = []
  ...
  function scheduleRetry() {
    ...
    retryTimer = setTimeout(() => { retrying = false; doConnect() }, 1000)
  }
  ...
  function doConnect() {
    ...
    socket.on('open', () => {
      ws = socket
      socket.send(JSON.stringify({ type: 'config', lang: lang || 'zh', model: 'base' }))
      ready = true
      for (const buf of pending) { try { socket.send(buf) } catch {} }
      pending.length = 0
    })
    ...
    socket.on('message', (data) => {
      try {
        ...
        if (msg.type === 'transcript' && msg.text) {
          onTranscript(msg.text, !!msg.is_final, msg.seg)
        }
      } catch {}
    })
    ...
    socket.on('error', (err) => {
      if (!closed && !ready) {
        ...
        onError(`Whisper 连接错误: ${err?.message || 'unknown'}`)
      }
    })
  ...
  // 自动启动 whisper_server.py（如果尚未运行），然后异步连接
  const vs = getVoiceStatus()
  if (vs.status !== 'running') {
    const result = startVoiceServer({ model: 'base' })
    if (result.status === 'error') {
      onError(`本地 Whisper 服务无法启动：${result.message}。请确认已安装 Python + openai-whisper + websockets。`)
      return null
    }
  }
  // 立即开始连接尝试（会重试直到 Python 服务就绪）
  doConnect()

  return {
    sendAudio(pcmBuffer) {
      const buf = Buffer.isBuffer(pcmBuffer) ? pcmBuffer : Buffer.from(pcmBuffer)
      if (!ready) { if (pending.length < 64) pending.push(buf); return }
...
// ─── 本地 FunASR 服务（ws://127.0.0.1:3723） ───
function createFunASRSession(lang, onTranscript, onError, onClose) {
  const FUNASR_WS_URL = 'ws://127.0.0.1:3723'
  let ws = null
  let ready = false
  let closed = false
  const pending = []
  ...
  function doConnect() {
    ...
    socket.on('error', (err) => {
      if (!closed && !ready) {
        ...
        onError(`FunASR 连接错误: ${err?.message || 'unknown'}`)
      }
    })
  ...
  // 自动启动语音服务（如果尚未运行），然后异步连接
  const vs = getVoiceStatus()
  if (vs.status !== 'running') {
    const result = startVoiceServer({ model: 'base' })
    if (result.status === 'error') {
      onError(`本地 FunASR 服务无法启动：${result.message}。请确认已安装 Python + FunASR + websockets。`)
      return null
    }
  }
  // 立即开始连接尝试（会重试直到服务就绪）
