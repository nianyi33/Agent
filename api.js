...
export function startAPI(port = 3721, { getStateSnapshot = null, onActivated = null } = {}) {
  ...
  const server = http.createServer(async (req, res) => {
      ...
      req.on('end', async () => {
        try {
          ...
          audioStream.on('data', (chunk) => {
            if (!headersWritten) {
              headersWritten = true
              const isPiper = creds.provider === 'piper-local'
              res.writeHead(200, {
                'Content-Type': isPiper ? 'audio/wav' : 'audio/mpeg',
    ...
    // POST /voice/install — 一键安装 Python + Whisper（SSE 流式进度）
    if (req.method === 'POST' && url.pathname === '/voice/install') {
      ...
      try {
        ...
        sse({ step: 'pip-install', message: '正在安装依赖包（openai-whisper + websockets + numpy）…', progress: 20 })
        execSync('python -m pip install openai-whisper websockets numpy --quiet', { stdio: 'pipe', windowsHide: true, timeout: 120000 })
        sse({ step: 'pip-done', message: '依赖包安装完成', progress: 40 })

        // 拷贝内置模型到 Whisper 缓存目录，无需联网下载
        sse({ step: 'copy-model', message: '正在部署语音识别模型…', progress: 50 })
        const whisperCache = join(process.env.USERPROFILE || process.env.HOME || tmpdir(), '.cache', 'whisper')
        fs.mkdirSync(whisperCache, { recursive: true })
        // 查找内置模型文件
        // 查找内置模型：打包路径 > 开发路径
        const unpackDir = process.env.BAILONGMA_RESOURCES_DIR?.replace('.asar', '.asar.unpacked')
        const bundledModel = (() => {
          const candidates = [
            unpackDir ? join(unpackDir, 'build', 'whisper-model', 'base.pt') : null,
            join(process.cwd(), 'build', 'whisper-model', 'base.pt'),
          ].filter(Boolean)
          return candidates.find(c => fs.existsSync(c))
        })()
        if (bundledModel) {
          fs.copyFileSync(bundledModel, join(whisperCache, 'base.pt'))
          sse({ step: 'model-done', message: '模型部署完成（从安装包，无需联网）', progress: 80 })
        } else {
          // 兜底：走镜像下载
          sse({ step: 'download-model', message: '正在下载模型（约 140MB）…', progress: 50 })
          execSync('python -c "import os;os.environ[\'HF_ENDPOINT\']=\'https://hf-mirror.com\';import whisper;whisper.load_model(\'base\')"', { stdio: 'pipe', windowsHide: true, timeout: 180000 })
          sse({ step: 'model-done', message: '模型下载完成', progress: 80 })
        }

        sse({ step: 'start-server', message: '正在启动语音识别服务…', progress: 90 })
        const result = startVoiceServer({ model: 'base' })
        if (result.status === 'error') {
          sse({ step: 'error', error: result.message, progress: 0 })
        } else {
          sse({ step: 'loading-model', message: '正在加载语音模型到内存（约 10 秒）…', progress: 92 })
          // 轮询等待服务真正就绪
          let attempts = 0
          while (attempts < 30) {
            ...
            if (vs.status === 'running') {
              sse({ step: 'done', message: '安装完成！语音识别已启动，可以说话了', ...vs, progress: 100 })
              break
            }
            if (vs.status === 'error') {
              sse({ step: 'error', error: vs.message, progress: 0 })
              break
            }
            attempts++
            if (attempts % 3 === 0) sse({ step: 'loading-model', message: '模型加载中…已等待 ' + attempts + ' 秒', progress: 92 })
          }
          if (attempts >= 30) sse({ step: 'error', error: '模型加载超时，请重试', progress: 0 })
        }
      } catch (err) {
