    ...
    <div style={{ height:'100%', overflowY:'auto', padding:'32px 36px', display:'flex', flexDirection:'column' }}>
      <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:24 }}>
        <div style={{ display:'flex', alignItems:'center', gap:10 }}>
          ...
          <h1 style={{ fontSize:22, fontWeight:700, color:'#F0F0FF', margin:0 }}>记忆宇宙</h1>
        </div>
        <div style={{ display:'flex', alignItems:'center', gap:8, background:'rgba(255,255,255,0.04)', border:'1px solid rgba(150,150,255,0.15)', borderRadius:12, padding:'8px 14px' }}>
          <Search size={14} style={{ color:'#555588' }} />
          <input value={search} onChange={e => setSearch(e.target.value)} placeholder="搜索记忆..."
            style={{ background:'transparent', border:'none', outline:'none', color:'#F0F0FF', fontSize:12, width:160 }} />
        </div>
      </div>

      {loading ? (
        <div style={{ textAlign:'center', padding:40, fontSize:13, color:'#555588' }}>加载中...</div>
      ) : memories.length === 0 ? (
        <GlassPanel variant="light" padding>
          <div style={{ textAlign:'center', padding:32 }}>
            <Brain size={32} style={{ color:'#555588', marginBottom:12 }} />
            <div style={{ fontSize:14, color:'#8888BB' }}>{search ? '没有匹配的记忆' : '记忆库为空'}</div>
            <div style={{ fontSize:11, color:'#555588', marginTop:4 }}>{search ? '尝试其他关键词' : '与 AI 对话后记忆会自动写入'}</div>
          </div>
        </GlassPanel>
      ) : (
          ...
          {memories.map(m => (
            <GlassPanel key={m.id} variant="light" padding>
              <div style={{ display:'flex', alignItems:'flex-start', gap:12 }}>
                <Calendar size={14} style={{ color:'#555588', marginTop:2, flexShrink:0 }} />
                <div style={{ flex:1, minWidth:0 }}>
                  <div style={{ fontSize:13, fontWeight:600, color:'#C0C0EE', marginBottom:4 }}>{m.title || '未命名记忆'}</div>
                  <div style={{ fontSize:11, color:'#8888BB', lineHeight:1.5, overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>
                ...
                <span style={{ fontSize:10, color:'#555588', flexShrink:0 }}>{new Date(m.created_at).toLocaleDateString('zh-CN')}</span>
              </div>
            </GlassPanel>
          ))}
        </div>
