...
>
> 项目:TreeSloth AI Agent · v1 草案 · 2026-06-23
>
...
#### `awakening` *(领域 kind)*
- **用途**:觉醒期探索反馈。单个 surface 逐条 morph 过每次发现(`index` 递增),觉醒结束时移除。建议 `intent: ambient`。
- **data**:
  ```jsonc
  { "index": 1, "total": 2, "title": "string", "finding": "string?", "emoji": "string?" }
  ```
- **上行 intent**:无(生命周期由 Agent 在觉醒末尾 `ui_set(remove)` 收起)。
