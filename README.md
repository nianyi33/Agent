![AI Agent](https://github.com/xiaoyuanda666-ship-it/TreeSloth AI Agent/blob/main/images/AGI128k.jpg)

# AI Agent

AI Agent 是一个持续运行的桌面 AI Agent 项目。它不是一次问答结束就退出的聊天程序，而是由主循环驱动：有用户消息时优先处理，空闲时按节奏继续整理记忆、检查任务、刷新上下文，并把状态实时推送到 Brain UI。
...
- 多模型接入：通过 xinyuntoken.com 聚合平台接入 DeepSeek、GLM、Kimi、Qwen、MiMo 等模型。
...
- 语音能力：支持云端语音识别和多种 TTS 服务（默认不启用入口，可在设置中开放）。
- 社交连接器：支持 Discord 与微信桥接（默认不启用入口，可在设置中开放）。
...
AI Agent 的长期状态主要保存在本地 SQLite 数据库中，包括：
